    ...
    public class TouchTracker
    {
        ...
        public void Trigger_TouchScreenConcept()
        {
            ...
            if (blobDetected)
            {
                ...
                cursorPosition.X = (int)(cursorPosition.X * calibrationFactorX);
                cursorPosition.Y = (int)(cursorPosition.Y * calibrationFactorY);
                // check limitations of detected position
                if (cursorPosition.X < 0 || cursorPosition.X > screenWidth ||
                    cursorPosition.Y < 0 || cursorPosition.Y > screenHeight)
                {
                    // point is out of region of interest
                    Console.WriteLine("Out:    " + cursorPosition.X + ", " + cursorPosition.Y);
                    return;
                }
                ...
                if (!mouseDown)
                {
                    mouseDown = true;
                    // add new contact
                    touchDriver.SendContact(1, cursorPosition.X, cursorPosition.Y, HidContactState.Adding);
                    Console.WriteLine("New     " + cursorPosition.X + ", " + cursorPosition.Y);
                }
                else if (cursorPosition.X != cursorLastX && cursorPosition.Y != cursorLastY)
                {
                    // move contact
                    ...
                    cursorLastX = cursorPosition.X;
                    cursorLastY = cursorPosition.Y;
                }
            }
            else if (mouseDown)
            {
                mouseDown = false;
                // remove contact
                touchDriver.SendContact(1, cursorPosition.X, cursorPosition.Y, HidContactState.Removing);
                Console.WriteLine("Remove: " + cursorPosition.X + ", " + cursorPosition.Y);
            }
        }
        ...
        public void CalculateCalibrationParameters()
        {
            ...
            calibrationFactorX = screenWidth / (double)width;
            calibrationFactorY = screenHeight / (double)height;

            frame.SetROI(calibrationROI);
            grayFrame.SetROI(calibrationROI);
            labelFrame.SetROI(calibrationROI);
